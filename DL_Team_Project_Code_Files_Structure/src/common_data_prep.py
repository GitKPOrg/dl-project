...
def prepare_data(
    ...
    downsample_val_target: Optional[int] = None,
    downsample_test_target: Optional[int] = None,
    max_words: int = 256,
...
#res_prep
    ...
    if downsample_val_target is not None and len(val) > int(downsample_val_target):
        val = downsample(val, target_size=int(downsample_val_target), col_stratified="label3", seed=seed)
    print("val columns:", val.column_names)
    print("length of val after downsampling",len(val))
    print("class counts of downsampled val data",val.to_pandas()["label3"].value_counts())

    if downsample_test_target is not None and len(test) > int(downsample_test_target):
        test = downsample(test, target_size=int(downsample_test_target), col_stratified="label3", seed=seed)
    print("test columns:", test.column_names)
    print("length of test after downsampling",len(test))
    print("class counts of downsampled test data",test.to_pandas()["label3"].value_counts())
...
#
    ...
    #
    preproc_csv_train = os.path.join(data_root, "data_train.csv")
    print("pre-processed csv file path for data_train join completed")

    preproc_csv_val = os.path.join(data_root, "data_val.csv")
    print("pre-processed csv file path for data_val join completed")

    preproc_csv_test = os.path.join(data_root, "data_test.csv")
    ...
    save_hf_dataset_to_csv(train_bal, preproc_csv_train, columns=available_cols)
    print("pre-processed csv file for train_data saved into data folder")
    ...
    # Save only columns that actually exist in val to avoid KeyError
    desired_cols = ["text", "rating", "label3", "label5", "lang"]
    available_cols = [c for c in desired_cols if c in val.column_names]
    if len(available_cols) < len(desired_cols):
        missing = [c for c in desired_cols if c not in available_cols]
        print(f"Note: missing columns {missing} — will save only available columns: {available_cols}")

    save_hf_dataset_to_csv(val, preproc_csv_val, columns=available_cols)
    print("pre-processed csv file for val_data saved into data folder")
    
    
    # Save only columns that actually exist in test to avoid KeyError
    desired_cols = ["text", "rating", "label3", "label5", "lang"]
    available_cols = [c for c in desired_cols if c in test.column_names]
    if len(available_cols) < len(desired_cols):
        missing = [c for c in desired_cols if c not in available_cols]
        print(f"Note: missing columns {missing} — will save only available columns: {available_cols}")

    save_hf_dataset_to_csv(test, preproc_csv_test, columns=available_cols)
    print("pre-processed csv file for test_data saved into data folder")

    #12v2
    #     # 12) save preprocessed CSVs (train / val / test)
    # preproc_train_csv = os.path.join(data_root, "data_train.csv")
    # preproc_val_csv   = os.path.join(data_root, "data_val.csv")
    # preproc_test_csv  = os.path.join(data_root, "data_test.csv")

    # desired_cols = ["text", "rating", "label3", "label5", "lang"]

    # # helper to compute available cols and save a dataset
    # def save_if_possible(dataset, out_path, name="dataset"):
    #     available = [c for c in desired_cols if c in dataset.column_names]
    #     if len(available) < len(desired_cols):
    #         missing = [c for c in desired_cols if c not in available]
    #         print(f"Note: {name} missing columns {missing} — will save only: {available}")
    #     save_hf_dataset_to_csv(dataset, out_path, columns=available)
    #     print(f"Saved pre-processed {name} CSV: {out_path}")

    # save_if_possible(train_bal, preproc_train_csv, name="train_bal")
    # save_if_possible(val, preproc_val_csv, name="val")
    # save_if_possible(test, preproc_test_csv, name="test")

    # # put the CSV path you consider "preproc_csv" into the return dict
    # preproc_csv = preproc_train_csv



    # create a run_outdir (simple timestamp)
    run_outdir = os.path.join(output_root, f"prep_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}")
    ...
    return {
        "run_outdir": run_outdir,
        "raw_csv": raw_csv,
        "preproc_csv_train": preproc_csv_train,
        "preproc_csv_val": preproc_csv_val,
        "preproc_csv_test": preproc_csv_test,
